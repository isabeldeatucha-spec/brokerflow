...
Both the PDF generator (agents/_shared/pdf_generator.py) and the in-app
side-panel preview (ui/queue_view._render_doc_panel) consume the dicts
returned here. Storing the raw structured data — not pre-rendered HTML
or PDF — means the in-app view always matches what gets generated as
the forwardable PDF artifact.

For new agents going live, write a payload generator that produces a
dict in the same shape as the seed entries below and register it
through register_payload(). The seed entries cover the demo cards.
"""
...
# Card → list of (doc_type, agent_for_doc) — drives both seed PDF
# generation and the side-panel preview.
def docs_for_card(card_id: str) -> list[tuple[str, str]]:
    out = []
    for (cid, doc_type) in _SEED_PAYLOADS:
        if cid == card_id:
            agent_for_doc = (
            ...
            out.append((doc_type, agent_for_doc))
    return out


def get_payload(card_id: str, doc_type: str) -> dict | None:
    """Return the structured data backing both the PDF and the
    in-app preview. None if the card/doc combo isn't registered."""
    return _SEED_PAYLOADS.get((card_id, doc_type))


def all_seed_entries() -> list[tuple[str, str, str, dict]]:
    """Yields (card_id, doc_type, agent, payload) for every seed card.
    Used by the lazy seed PDF generator."""
    out = []
    for (card_id, doc_type), payload in _SEED_PAYLOADS.items():
        ...
        out.append((card_id, doc_type, agent, payload))
    return out
